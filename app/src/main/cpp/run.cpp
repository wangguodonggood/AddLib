...
#define STB_IMAGE_IMPLEMENTATION  //---------- no use
#include "stb_image.h"
...
#define GRAPH_FILE_NAME "/sdcard/mvnc/graph"

// image file name - assume we are running in this directory: ncsdk/examples/caffe/GoogLeNet/cpp
#define IMAGE_FILE_NAME "/sdcard/mvnc/cat.jpg"
#define IMAGE0_FILE_NAME "/sdcard/mvnc/goose.jpg"
...
float networkMean[] = {0.40787054*255.0, 0.45752458*255.0, 0.48109378*255.0};  //----- why
...
half *LoadImage(const char *path, int reqsize, float *mean)
{
	...
	img = stbi_load(path, &width, &height, &cp, 3);  //return null ptr
	if(!img)
	{
		ALOGE("The picture %s could not be loaded\n", path);
		return 0;
	}
	imgresized = (unsigned char*) malloc(3*reqsize*reqsize);  //---------------why
	if(!imgresized)
	{
		free(img);
		perror("malloc");
...
int run()
{
    ...
    {   // error allocating graph
        ALOGE("Could not allocate graph for file: %s\n", GRAPH_FILE_NAME);
        ALOGE("Error from mvncAllocateGraph is: %d\n", retCode);
    }
    else
    {   // successfully allocated graph.  Now graphHandle is ready to go.  
        ...
        // when done with it.
        ALOGE("Successfully allocated graph for %s\n", GRAPH_FILE_NAME);
        ...
        {   // error loading tensor
            ALOGE("Could not load tensor\n");
            ALOGE("Error from mvncLoadTensor is: %d\n", retCode);
        }
        else
        {   // the inference has been started, now call mvncGetResult() for the
	        ...
	        resultData32 = (float*)malloc(numResults * sizeof(*resultData32));
                ...
                ALOGE("---------------------Index of top result is: %d\n", maxIndex);
                ALOGE("---------------------Probability of top result is: %f\n", resultData32[maxIndex]);
            }
            ALOGE("#####################has run to here to load second image\n");
            half* imageBufFp16 = LoadImage(IMAGE0_FILE_NAME, networkDim, networkMean);

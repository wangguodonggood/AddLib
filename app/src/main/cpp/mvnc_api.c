...
static mvncStatus load_fw_file(const char *name)
{
	...
	Dl_info info;
    //mvncOpenDevice函数所在的地址  我这里是编译到了libnative_lib.so这个文件中
	dladdr(mvncOpenDevice, &info);
    //mv_cmd_file  返回的是“libnative_lib.so”
    ...
    fp = fopen("/sdcard/mvnc/MvNCAPI.mvcmd", "rb+w");
//    fp = fopen("/sdcard/mvnc/test.txt", "w+");
    ALOGE("\nfopen----->!,fp = %c", fp[0] );
    ...
    ALOGE("usb_boot!, rc=%d",rc);
	free(tx_buf);
	if (rc) {
		pthread_mutex_unlock(&mm);
		return rc;
	}
    ALOGE("Boot successful, device address %s\n", name);
	return MVNC_OK;
}
...
mvncStatus mvncOpenDevice(const char *name, void **deviceHandle)
{
    ...
    ALOGE("Excute!,rc=%d\n , mvnc-api--->mvncOpenDevice",rc);
    if (saved_name && strlen(saved_name) > 0) {
        ALOGE("Excute!,rc=%d\n , mvnc-api--->mvncOpenDevice",(saved_name && strlen(saved_name)));

        device_name = strtok_r(NULL, ":", &saved_name);
		second_name_available = 1;
	}
    ALOGE("Excute!,run to here \n");
//    pthread_mutex_unlock(&mm);//add by liang@topotek;for test mvnc;
//    return MVNC_OK; //add by liang@topotek;for test mvnc;
    for(int i=0; i<8; i++) {
        usleep(1000000);
        usleep(1000000);
        usleep(1000000);
                              ...
                              const void *graphFile, unsigned int graphFileLength)
{
    ...
    ALOGE("run to here;device match noutputs=%d\n",noutputs);
    ...
    ALOGE("run to here;device match d=%d\n",d);
	...
	do {
		...
		usleep(10000);
        ALOGE("run to here;myriadstatus = %d\n",status);
    ...
    ALOGE("run to here 505,usblink_setdata\n");
    ...
    ALOGE("run to here,usblink_setdata finish\n");

	g->debug_buffer = g->aux_buffer;
	g->time_taken = (float *) (g->aux_buffer + 224);

    ALOGE("run to here,calloc star\n");
	// output_data
	g->output_data = calloc(noutputs, 2);

    ALOGE("run to here,calloc finish\n");
